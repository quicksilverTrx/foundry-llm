...
def main():
    ...
    torch.manual_seed(0)
    pkg_dir = Path("artifacts/p2_bpe_smoke/4")
    ...
    tok_cfg = SubwordTokenizerConfig(vocab_size=10000, model_type="bpe")
    ...
    ids_dir = pkg_dir / "token_ids"
    ids_dir.mkdir(parents=True, exist_ok=True)
    torch.save(torch.tensor(train_ids, dtype=torch.long), ids_dir / "train_ids.pt")
    torch.save(torch.tensor(val_ids, dtype=torch.long), ids_dir / "val_ids.pt")
    print(f"Saved token id splits to: {ids_dir}")
