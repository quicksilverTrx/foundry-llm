...
- **Mar–Apr 2026 (serving)** — the question shifts from "can we train it" to "can we serve it with measured latency." KV-cache engine with prefill/decode separation; correctness proven before optimisation (cache equivalence gate: max |logit_diff| 2.09e-05 at atol 1e-4 — no performance work started until this gate was green). Measured **11× decode speedup** on NanoLlama 8L (17.8 ms/tok cached vs 196 ms/tok recompute, prompt=256). TTFT/TPS grid across ctx=256/512/1024 × B=1/2/4 revealed two empirical patterns: TTFT scales as O(context²) (compute-bound prefill), while decode TPS is flat across batch sizes (memory-bandwidth-bound weight loading) — meaning prefill batching is essentially free at the decode stage. int8 dynamic quantisation: 70% memory reduction (486 MB → 147 MB) at +6% PPL drift, 1.27× faster decode — but 2.9× slower prefill, because dynamic quantisation helps where the bottleneck is memory bandwidth (decode) and hurts where it is arithmetic throughput (prefill). The GQA choice made during training (4 KV heads vs 12, for single batched SDPA: 38K→60K tok/s) compounds into a serving dividend: 3× smaller KV weight matrices per decode step and 16 MB KV-cache at block_size=1024 vs 48 MB for MHA — the same architectural decision pays off twice. FastAPI with SSE streaming, batched prefill, rate limiting, PII/profanity safety filters, tiktoken adapter for NanoLlama. Prompt suite, perplexity regression, precision policy with explicit device-fallback reason strings. 152 tests.
